    ...
    <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row items-center">
      ...
      <div class="md:ml-8">
        ...
        <div class="mt-12">
          ...
          <div class="flex mt-4">
            <div
              v-for="crew in props.movieDetails?.crew?.slice(0, 5)"
              :key="crew.name"
              class="mr-8"
            >
              <div>{{ crew.name }}</div>
        ...
        <div>
          <template v-if="(props.movieDetails?.videos?.results ?? []).length > 0">
            ...
            <!-- Begin Trailer Modal -->
            <template v-if="isMovieTrailerModalOpen">
              <div
...
function closeMovieTrailerModal(): void {
  if (isMovieTrailerModalOpen.value) {
    isMovieTrailerModalOpen.value = false;
  }
}

async function openMovieTrailerModal(): Promise<void> {
  if (! isMovieTrailerModalOpen.value) {
    isMovieTrailerModalOpen.value = true;
    await nextTick();
    movieTrailerModal.value?.focus();
  }
}
</script>
